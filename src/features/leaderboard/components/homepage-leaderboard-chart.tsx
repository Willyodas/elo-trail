...
export function HomepageLeaderboardChart({
  ...
  const chartSummary = orderedPlayers
    .map((player) => {
      const firstPoint = player.points[0];
      const lastPoint = player.points.at(-1);

      if (!firstPoint || !lastPoint) {
        return `Rank ${player.rank}, ${player.name}, has no matchmaking ELO history in this period.`;
      }
      ...
      return `Rank ${player.rank}, ${player.name}, started at ${firstPoint.rating.toLocaleString()} ELO and ended at ${lastPoint.rating.toLocaleString()} ELO, ${direction} ${Math.abs(change).toLocaleString()} points.`;
    })
    .join(" ");

  return (
    <div className="space-y-4">
      <p id="leaderboard-chart-summary" className="sr-only">
        {chartSummary}
      </p>

      <div
        className="flex flex-wrap gap-x-4 gap-y-2 text-xs text-black/65 dark:text-white/65"
        ...
        {orderedPlayers.map((player, index) => (
          ...
          >
            <span
              ...
              style={{ backgroundColor: SERIES_COLOURS[index] }}
              aria-hidden="true"
            />
      ...
      <div
        ...
        aria-describedby="leaderboard-chart-summary"
      >
